...
winctl.FindWindows(win => win.isVisible() && win.getTitle()).then(windows => {
	...
	o[window.getTitle()] = new Object();
	o[window.getTitle()]['pid'] = window.getPid();
	o[window.getTitle()]['hwnd'] = window.getHwnd();
	o[window.getTitle()]['parent'] = window.getParent();
	o[window.getTitle()]['size'] = window.dimensions();
	o[window.getTitle()]['client'] = window.getClientRect();
	o[window.getTitle()]['position'] = window.clientToScreen();
	//console.log(" - %s [pid=%d, hwnd=%d, parent=%d]", window.getTitle(), window.getPid(), window.getHwnd(), window.getParent(),window.dimensions())
                 }
	);
